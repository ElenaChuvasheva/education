...
class ViewTest(APITestCase, URLPatternsTestCase):
    ...
    def test_post_full_created(self):
        ...
        self.client.post(
            ...
            format='json')
        new_object_qset = EducationalModule.objects.filter(
            name='test post full created')
        self.assertTrue(new_object_qset.exists())
        new_object_qset.delete()
    ...
    def test_post_full_content(self):
        ...
        response = self.client.post(
            reverse('api:module-list'), data, format='json')
        content = json.loads(response.content)
        new_object = EducationalModule.objects.get(
            name='test post full content')
    ...
    def test_post_short_created(self):
        ...
        self.client.post(reverse('api:module-list'),
                         ...
                         format='json')
        created_object = EducationalModule.objects.filter(
            name='test post short created')
    ...
    def test_patch_name_content(self):
        '''Метод PATCH возвращает правильный контент с изменённым именем'''
        response = self.client.patch(
            reverse('api:module-detail',
                    kwargs={'pk': ViewTest.test_module.pk}),
            {'name': 'patched name'}, format='json')
        content = json.loads(response.content)
    ...
    def test_patch_description_db(self):
        ...
        self.client.patch(
            reverse('api:module-detail',
                    kwargs={'pk': ViewTest.test_module.pk}),
            {'description': 'patched description'}, format='json')
        ViewTest.test_module.refresh_from_db()
        self.assertEqual(
            ViewTest.test_module.description, 'patched description')
        ViewTest.test_module.description = ViewTest.test_module_data[
            'description']
        ViewTest.test_module.save()

    def test_patch_description_content(self):
        '''Метод PATCH возвращает правильный контент с изменённым описанием'''
        response = self.client.patch(
            reverse('api:module-detail',
                    kwargs={'pk': ViewTest.test_module.pk}),
            {'description': 'patched description'}, format='json')
    ...
    def test_post_delete_no_object(self):
        ...
        self.client.delete(
            reverse('api:module-detail',
                    kwargs={'pk': ViewTest.test_module.pk}))
        deleted_object_exists = EducationalModule.objects.filter(
            pk=ViewTest.test_module.pk).exists()
    ...
    def test_post_short_status(self):
        ...
        response = self.client.post(
            reverse('api:module-list'),
            {'name': 'test post short status'}, format='json')
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
    ...
    def test_put_status(self):
        ...
        response = self.client.put(
            reverse('api:module-detail',
                    kwargs={'pk': ViewTest.test_module.pk}),
            {'name': 'put name', 'description': 'put description'},
            format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ViewTest.test_module.save()
